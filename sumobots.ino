...
void loop() {
	...
	while (buttonPressed == false) {
      ...
      delay(200);
		...
		int front =
		    getUltraSonicReading(trigUltraSonic1_PIN, echoUltraSonic1_PIN);
		int left =
		    getUltraSonicReading(trigUltraSonic2_PIN, echoUltraSonic2_PIN);
		int right =
		    getUltraSonicReading(trigUltraSonic3_PIN, echoUltraSonic3_PIN);
...
int getUltraSonicReading(int trigUltraSonicPin, int echoUltraSonicPin) {
	...
	delayMicroseconds(2);
	digitalWrite(trigUltraSonicPin,
	             HIGH);  // Sets the trigPin HIGH (ACTIVE) for 10 microseconds
	delayMicroseconds(10);
	digitalWrite(trigUltraSonicPin, LOW);
	duration = pulseIn(
	    echoUltraSonicPin,
	    HIGH);  // Reads the echoPin, returns the sound wave travel time in microseconds
	distance = (duration * 0.0340 / 2) +
	           2;  // Speed of sound wave divided by 2 (go and back)
